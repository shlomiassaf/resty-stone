...
In such cases, add an "__extends__" property to the profile you wish to extend and set the value to the name of the parent profile. 
...
In the example above, isAdmin has the same behavior as default in the output structure, the also both accept 'get' HTTP VERBS.
However, isAdmin accepts post, put and delete command.
...
Layer 3 & 4 a functions implemented on an instance of a `resty-stone` class called `RestList`.
...
and a callback and returns a result.  
The result is sent to a transformation handler and then sent back to the client.
...
> - RPC does not care if it was called on a Resource group or on a Resource group, this logic should be handled in the Function handler.
...
To customize a `RestList` simply create a new instance of it (`new RestList()`), overwrite or set new functions that customize a resource to your desire.  
...
By working with instances of `RestList` default behavior is kept unless specified other wise.
