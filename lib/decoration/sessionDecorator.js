 ...
 * @param keystone
 */
function sessionDecorator(keystone) {
    // save token header to be used in cookieParserDecorator middleware.
    tokenHeaderName = keystone.get('resty token header');

    keystone.session = RestySession.factory(keystone);
    keystone.set('session', keystone.session.persist);
};
...
function keystoneOnMount(){
    ...
    if (keystone.session.disableCookies === true) {
        var sessionOptions = keystone.get('session options');
        for(var i=0; i < keystone.app.stack.length; i++){
            if (keystone.app.stack[i].handle === sessionOptions.cookieParser) {
                ...
                originalSessionMiddleware = keystone.app.stack[i+1].handle

                // TODO: on move to express 4.0 put regex here instead of restBaseMiddlewareGenerator...
                keystone.app.use("/", cookieParserDecorator);
                keystone.app.stack.splice(i, 2, keystone.app.stack.pop()); // remove 2, cookieParser & express.session
            }
        }
    }
}
